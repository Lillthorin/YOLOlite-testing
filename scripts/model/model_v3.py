...
def init_detect_bias(head_moduledict, num_classes, p_init=0.01):
    """
    [ÄNDRAD] Initierar bias för NMS-fri detektion (inget obj-head).
    Sätter cls-bias lågt så att nätverket förutspår bakgrund i början.
    """
    # Vi använder samma 'prior probability' logik på cls som vi tidigare gjorde på obj.
    # Sigmoid(bias) ≈ p_init
...
def make_head(A, head_depth, C, fpn_channels):
    """
    [ÄNDRAD] Head med depthwise conv + ReLU.
    Tar bort 'obj'-grenen för att stödja NMS-fri arkitektur.
    """
    trunk = nn.Sequential(*[
    ...
    out_layers = nn.ModuleDict({
        ...
        # "obj": nn.Conv2d(fpn_channels, A * 1, 1),  <-- BORTTAGEN
...
class YOLOLiteMS(nn.Module):
    def __init__(
        ...
        num_classes=3,
        fpn_channels=128,
        # [ÄNDRAD] Default till 1 ankare per nivå för NMS-fri logik
    ...
    def _forward_head(self, p, head_dict, A):
        """[ÄNDRAD] Inget 'obj' output längre."""
        p = head_dict["trunk"](p)
        
        box = head_dict["out"]["box"](p)
        # obj = head_dict["out"]["obj"](p) # <-- BORTTAGEN
        ...
        # Concatenera [box, cls]. Output blir [B, A, 4+C, S, S]
...
class YOLOLiteMS_CPU(nn.Module):
    def __init__(
        ...
        num_classes=3,
        fpn_channels=96,
        # [ÄNDRAD] Default till 1 ankare per nivå
        num_anchors_per_level=(1,1,1,1),
        pretrained=True,
        depth_multiple=0.75,
        width_multiple=0.75,
